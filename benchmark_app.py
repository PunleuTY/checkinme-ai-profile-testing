...
def build_experimental_prompt(
    ...
    prompt = (
        ...
        f"individual. "
        f"Keep the SAME hairstyle, with identical length, cut, parting, hairline, natural hair color, and "
        ...
        f"Replace the clothing with conservative, formal corporate business attire suitable for an "
        f"official ID / corporate headshot. Dress the subject in "
        f"{outfit_style}. The clothing must look fully professional, well-fitted, clean, crisp, and "
        f"wrinkle-free, with structured tailoring and no casual, trendy, fashion-editorial, or "
        f"non-professional elements. Keep the outfit exactly as described, with no added layers, "
        f"prints, or decorative details. Remove any non-professional items "
        f"the person is wearing in the original "
        f"photo, such as earphones, earbuds, headphones, hats, caps, sunglasses, lanyards, casual or "
        ...
        f"and shoulders framing. Crop from the top of the head to the mid-chest only. This must be a "
        f"close head-and-chest portrait, NOT a full-body, half-body, or long/wide shot; do not zoom "
        f"out to show the waist, hips, or full body. NO hands, "
        f"arms, fingers, or hand gestures anywhere in the frame; the subject's arms must not be "
        f"visible and the pose must be a still, neutral head-and-shoulders portrait with no gesturing. "
        f"Centered composition, neutral confident expression. Reposition the subject to face the "
        f"camera directly in a perfectly straight-on, front-facing pose, with head level and upright "
        ...
        f"and neckline sitting naturally against the neck, with no floating fabric or gaps, neck and "
    ...
    if is_female:
        prompt += (
            ...
            "original, whether loose or tied up, with the same ponytail, bun, or updo if present, the same "
            ...
            "ATTIRE (strict): reproduce the requested outfit and accessories EXACTLY, with the same "
            ...
            "the collarbone, with conservative coverage, no plunging or low-cut neckline, no exposed "
